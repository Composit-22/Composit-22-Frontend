...
const Event = () => {
    ...
    return (
        ...
        <EventBody title={event.title} desc={event.desc} id={event.id} colorId={event.id % 6} min={event.min} max={event.max} participant_info = {event.participant_info} Details={event.Details} submit_link={event.submit_link}/>
    );
};
